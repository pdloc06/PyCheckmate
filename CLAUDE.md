...
**`engine/movegen.py`** — legal move generation, as a **free function taking a `GameState`**, not a method: `generate_legal(gs, for_ai=..., captures_only=...)`. Quiescence uses the `captures_only=True` form directly; a `generate_captures()` wrapper existed briefly and was deleted as dead code. Free functions are what let `board` stay ignorant of `movegen`; a mixin would have forced an import back the other way. There are **two parallel move pipelines**:
